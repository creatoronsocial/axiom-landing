...
export const metadata: Metadata = {
  title: 'Client Switching Pack | Axiom Labs',
  description:
    'Switch client context cleanly between agency-mode engagements. Reset assumptions, isolate working state, and move between accounts without carrying stale context. Live on Claw Mart.',
};
...
const pillars = [
  {
    icon: '🧼',
    title: 'Clean resets',
    description: 'Use a repeatable reset routine before each new client session so stale assumptions do not bleed into the next account.',
  },
  {
    icon: '🗂️',
    title: 'Context boundaries',
    description: 'Separate notes, tasks, and active decisions so each client workspace keeps its own operating state.',
  },
  {
    icon: '🔁',
    title: 'Reliable switching',
    description: 'Move between agency-mode clients with a predictable handoff sequence instead of rebuilding your mental model every time.',
  },
];

const benefits = [
  {
    title: 'Fewer cross-client mistakes',
    description: 'The pack reduces accidental carryover when operators jump between multiple active accounts.',
  },
  {
    title: 'Faster session starts',
    description: 'A clean switching checklist means less re-orientation before real work begins.',
  },
  {
    title: 'More trustworthy execution',
    description: 'Each client engagement starts from the right context, with less guesswork and less cleanup later.',
  },
];

export default function ClientSwitchingPage() {
  return (
    <main className="relative overflow-hidden bg-[var(--background)] text-[var(--foreground)]">
        ...
        <div className="relative grid flex-1 items-center gap-12 py-14 lg:grid-cols-[1.05fr_0.95fr] lg:gap-16 lg:py-20">
          <div className="max-w-3xl">
            ...
            <h1 className="max-w-4xl text-5xl font-semibold tracking-[-0.06em] text-slate-950 sm:text-6xl lg:text-7xl">
              Client Switching Pack
            </h1>

            <p className="mt-6 max-w-2xl text-lg leading-8 text-slate-600 sm:text-xl">
              Switch client context cleanly between agency-mode engagements. Reset assumptions, isolate working state, and move between accounts without carrying stale context.
            </p>

            <div className="mt-4 flex items-center gap-4">
              <span className="rounded-full bg-slate-100 px-3 py-1 text-sm font-semibold text-slate-700">$19</span>
              <span className="text-sm text-slate-500">One-time purchase</span>
            </div>

            <div className="mt-10 flex flex-col gap-4 sm:flex-row">
              <a
                href="https://www.shopclawmart.com/listings/client-switching-pack-agency-mode-67d76e58"
            ...
            <div className="relative overflow-hidden rounded-[2rem] border border-white/70 bg-[linear-gradient(160deg,rgba(255,255,255,0.96),rgba(244,248,255,0.88))] p-5 shadow-[0_30px_90px_rgba(76,94,129,0.16)] backdrop-blur">
              ...
              <div className="relative rounded-[1.6rem] border border-slate-100 bg-white/90 p-5 shadow-inner shadow-slate-100/80">
                <div className="flex items-center justify-between border-b border-slate-100 pb-4">
                  <div>
                    <p className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-400">Switching flow</p>
                    <h2 className="mt-2 text-2xl font-semibold tracking-[-0.04em] text-slate-950">Reset, separate, resume</h2>
                  </div>
                ...
                <div className="mt-6 space-y-4">
                  <SwitchStep icon="🧼" title="Clear stale context" status="Reset" />
                  <div className="mx-auto h-6 w-px bg-slate-200" />
                  <SwitchStep icon="🗂️" title="Load the right client state" status="Separate" />
                  <div className="mx-auto h-6 w-px bg-slate-200" />
                  <SwitchStep icon="🔁" title="Resume with confidence" status="Restart" />
                </div>

                <div className="mt-6 rounded-2xl border border-amber-100 bg-amber-50/60 p-4">
                  <p className="text-sm font-medium text-amber-800">
                    If the client context feels fuzzy, stop and reset first. Fast switching is only useful when boundaries stay clean.
                  </p>
                </div>
              </div>
            </div>
      ...
      <section className="relative mx-auto w-full max-w-7xl px-6 py-16 sm:px-8 lg:px-10">
        <div className="mb-10">
          ...
          <h2 className="mt-3 text-3xl font-semibold tracking-[-0.05em] text-slate-950 sm:text-4xl">
            Reset context before it leaks.
          </h2>
          <p className="mt-4 max-w-3xl text-lg leading-8 text-slate-600">
            The pack gives operators a repeatable switching routine so each agency-mode client starts from the right notes, active tasks, and assumptions.
          </p>
        </div>
      ...
      <section className="relative mx-auto w-full max-w-7xl px-6 py-16 sm:px-8 lg:px-10">
        <div className="rounded-[2rem] border border-emerald-200 bg-emerald-50/50 p-8 shadow-[0_25px_80px_rgba(34,197,94,0.08)] sm:p-10">
          <div className="max-w-2xl">
            <p className="text-sm font-semibold uppercase tracking-[0.2em] text-emerald-700">Live status</p>
            <h2 className="mt-3 text-3xl font-semibold tracking-[-0.05em] text-slate-950 sm:text-4xl">
              Now live on Claw Mart.
            </h2>
            <p className="mt-4 text-lg leading-8 text-slate-600">
              The pack is now publicly available on Claw Mart. Buy the live listing there while the rest of the shelf continues moving from early access to live status one product at a time.
            </p>
          </div>
...
function SwitchStep({ icon, title, status }: { icon: string; title: string; status: string }) {
  return (
