...
export const metadata: Metadata = {
  title: 'Task Intake & Scoping Pack | Axiom Labs',
  description:
    'Stop agents from wasting time on ambiguous tasks. Clean intake forms, clarity scoring, and before/after scoping examples. Live on Claw Mart.',
};
...
const pillars = [
  {
    ...
    description: 'Turn fuzzy requests into scoped work before an agent starts guessing its way through the assignment.',
  },
  {
    icon: '🧪',
    title: 'Clarity scoring',
    description: 'Use a simple rubric to spot missing inputs, hidden assumptions, and handoff risk before execution begins.',
  },
  {
    icon: '📝',
    title: 'Reusable intake patterns',
    description: 'Copy-paste templates and worked examples make good task definition repeatable instead of ad hoc.',
  },
];

const benefits = [
  {
    title: 'Less agent thrash',
    description: 'Better intake means fewer false starts, fewer vague loops, and less wasted execution time.',
  },
  {
    title: 'Stronger handoffs',
    description: 'Scoped tasks travel better between humans and agents because the expected outcome is already explicit.',
  },
  {
    title: 'Faster review cycles',
    description: 'When the task was clean at the start, review becomes simpler and less about reconstructing intent.',
  },
];

export default function TaskIntakeScopingPage() {
  return (
        ...
        <div className="relative grid flex-1 items-center gap-12 py-14 lg:grid-cols-[1.05fr_0.95fr] lg:gap-16 lg:py-20">
          <div className="max-w-3xl">
            ...
            <h1 className="max-w-4xl text-5xl font-semibold tracking-[-0.06em] text-slate-950 sm:text-6xl lg:text-7xl">
              Task Intake &amp; Scoping Pack
            </h1>

            <p className="mt-6 max-w-2xl text-lg leading-8 text-slate-600 sm:text-xl">
              Stop agents from wasting time on ambiguous tasks. Use clean intake forms, clarity scoring, and before/after examples to turn vague requests into executable work.
            </p>
            ...
            <div className="mt-10 flex flex-col gap-4 sm:flex-row">
              <a
                href="https://www.shopclawmart.com/listings/task-intake-scoping-pack-1af96426"
            ...
            <div className="relative overflow-hidden rounded-[2rem] border border-white/70 bg-[linear-gradient(160deg,rgba(255,255,255,0.96),rgba(244,248,255,0.88))] p-5 shadow-[0_30px_90px_rgba(76,94,129,0.16)] backdrop-blur">
              ...
              <div className="relative rounded-[1.6rem] border border-slate-100 bg-white/90 p-5 shadow-inner shadow-slate-100/80">
                <div className="flex items-center justify-between border-b border-slate-100 pb-4">
                  <div>
                    <p className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-400">Scoping flow</p>
                    <h2 className="mt-2 text-2xl font-semibold tracking-[-0.04em] text-slate-950">Intake, score, assign</h2>
                  </div>
                ...
                <div className="mt-6 space-y-4">
                  <FlowStep icon="📝" title="Capture the request" status="Intake" />
                  <div className="mx-auto h-6 w-px bg-slate-200" />
                  <FlowStep icon="🧪" title="Score missing clarity" status="Review" />
                  <div className="mx-auto h-6 w-px bg-slate-200" />
                  <FlowStep icon="🚀" title="Launch clean work" status="Assign" />
                </div>

                <div className="mt-6 rounded-2xl border border-amber-100 bg-amber-50/60 p-4">
                  <p className="text-sm font-medium text-amber-800">
                    If a task is still vague, do not start execution faster — tighten the intake first.
                  </p>
                </div>
              </div>
            </div>
      ...
      <section className="relative mx-auto w-full max-w-7xl px-6 py-16 sm:px-8 lg:px-10">
        <div className="mb-10">
          ...
          <h2 className="mt-3 text-3xl font-semibold tracking-[-0.05em] text-slate-950 sm:text-4xl">
            Cleaner tasks before agents touch the keyboard.
          </h2>
          <p className="mt-4 max-w-3xl text-lg leading-8 text-slate-600">
            The pack helps operators define work upstream so agents spend less time interpreting unclear asks and more time executing useful work.
          </p>
        </div>
        ...
        <div className="rounded-[2rem] border border-emerald-200 bg-emerald-50/50 p-8 shadow-[0_25px_80px_rgba(34,197,94,0.08)] sm:p-10">
          <div className="max-w-2xl">
            ...
            <p className="mt-4 text-lg leading-8 text-slate-600">
              The pack is now publicly available on Claw Mart. Buy the live listing there while the rest of the shelf keeps expanding one operator workflow at a time.
            </p>
          </div>
...
function FlowStep({ icon, title, status }: { icon: string; title: string; status: string }) {
  return (
