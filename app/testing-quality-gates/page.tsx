...
export const metadata: Metadata = {
  title: 'Testing & Quality Gates Pack | Axiom Labs',
  description: 'Zero broken commits from agents. A three-gate system (lint → test → review) that enforces quality before any agent change lands. Live on Claw Mart.',
};
...
const qualityGates = [
  {
    icon: '🔍',
    title: 'Lint Gate',
    description: 'Catches syntax errors, style violations, and common mistakes before any test runs.',
  },
  {
    icon: '✅',
    title: 'Test Gate',
    description: 'Runs unit and integration tests. No agent change lands with failing tests.',
  },
  {
    icon: '👀',
    title: 'Review Gate',
    description: 'Requires human or automated review. Agents cannot self-merge without oversight.',
  },
];

const benefits = [
  {
    title: 'Zero broken commits',
    description: 'The pack enforces all three gates in sequence. If any gate fails, the commit is blocked automatically.',
  },
  {
    title: 'Agent accountability',
    description: 'Every agent action is traceable through the gate chain. No silent failures or skipped checks.',
  },
  {
    title: 'Operator confidence',
    description: 'Wake up to working code, not broken builds. The gates run before changes land, not after.',
  },
];

export default function TestingQualityGatesPage() {
  return (
        ...
        <div className="relative grid flex-1 items-center gap-12 py-14 lg:grid-cols-[1.05fr_0.95fr] lg:gap-16 lg:py-20">
          <div className="max-w-3xl">
            ...
            <h1 className="max-w-4xl text-5xl font-semibold tracking-[-0.06em] text-slate-950 sm:text-6xl lg:text-7xl">
              Testing & Quality Gates Pack
            </h1>

            <p className="mt-6 max-w-2xl text-lg leading-8 text-slate-600 sm:text-xl">
              Zero broken commits from agents. A three-gate system that enforces lint → test → review before any agent change lands.
            </p>
            ...
            <div className="mt-10 flex flex-col gap-4 sm:flex-row">
              <a
                href="https://www.shopclawmart.com/listings/testing-quality-gates-pack-zero-broken-commits-from-agents-d7429f27"
            ...
            <div className="relative overflow-hidden rounded-[2rem] border border-white/70 bg-[linear-gradient(160deg,rgba(255,255,255,0.96),rgba(244,248,255,0.88))] p-5 shadow-[0_30px_90px_rgba(76,94,129,0.16)] backdrop-blur">
              ...
              <div className="relative rounded-[1.6rem] border border-slate-100 bg-white/90 p-5 shadow-inner shadow-slate-100/80">
                <div className="flex items-center justify-between border-b border-slate-100 pb-4">
                  <div>
                    <p className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-400">Quality pipeline</p>
                    <h2 className="mt-2 text-2xl font-semibold tracking-[-0.04em] text-slate-950">Three-gate enforcement</h2>
                  </div>
                ...
                <div className="mt-6 space-y-4">
                  <GateStep number={1} icon="🔍" title="Lint" status="First" />
                  <div className="mx-auto h-6 w-px bg-slate-200" />
                  <GateStep number={2} icon="✅" title="Test" status="Second" />
                  <div className="mx-auto h-6 w-px bg-slate-200" />
                  <GateStep number={3} icon="👀" title="Review" status="Final" />
                </div>

                <div className="mt-6 rounded-2xl border border-amber-100 bg-amber-50/60 p-4">
                  <p className="text-sm font-medium text-amber-800">
                    All gates must pass. If any gate fails, the commit is blocked and the agent is notified.
                  </p>
                </div>
              </div>
            </div>
      ...
      <section className="relative mx-auto w-full max-w-7xl px-6 py-16 sm:px-8 lg:px-10">
        <div className="mb-10">
          ...
          <h2 className="mt-3 text-3xl font-semibold tracking-[-0.05em] text-slate-950 sm:text-4xl">
            Three gates. Zero escape routes.
          </h2>
          <p className="mt-4 max-w-3xl text-lg leading-8 text-slate-600">
            Each gate is a hard requirement. Agents cannot skip, bypass, or self-approve past any failed gate.
          </p>
        </div>

        <div className="grid gap-6 md:grid-cols-3">
          {qualityGates.map((gate) => (
            <div key={gate.title} className="rounded-2xl border border-slate-200 bg-white p-6">
              ...
              <h3 className="text-lg font-semibold text-slate-950">{gate.title}</h3>
              <p className="mt-2 text-sm leading-6 text-slate-600">{gate.description}</p>
            </div>
          ))}
        </div>
      </section>
        ...
        <div className="rounded-[2rem] border border-emerald-200 bg-emerald-50/50 p-8 shadow-[0_25px_80px_rgba(34,197,94,0.08)] sm:p-10">
          <div className="max-w-2xl">
            ...
            <p className="mt-4 text-lg leading-8 text-slate-600">
              The pack is now publicly available on Claw Mart. Buy the live listing there, while the rest of the shelf moves from preparation to live status one product at a time.
            </p>
          </div>
...
function GateStep({ number, icon, title, status }: { number: number; icon: string; title: string; status: string }) {
  return (
    <div className="flex items-center gap-4">
      ...
      <div className="flex-1">
        ...
        <p className="text-xs text-slate-500">Gate {number} of 3</p>
      </div>
    </div>
  );
}
